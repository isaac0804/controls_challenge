...
def run_rollout(data_path, controller_type, model_path, controller_params, debug=False):
  tinyphysicsmodel = TinyPhysicsModel(model_path, debug=debug)
  controller = importlib.import_module(f'controllers.{controller_type}').Controller(*controller_params)
...
def grid_search_pid(model_path, data_path, num_segs, debug=False):
    # Define the range of PID parameters to search over
    # initial_kp = 0.3
    # initial_ki = 0.05
    # initial_kd = -0.1
    initial_kp = 0.1732
    ...
    kp_range = np.logspace(np.log10(initial_kp / range), np.log10(initial_kp * range), num_points)
    ki_range = np.logspace(np.log10(initial_ki / range), np.log10(initial_ki * range), num_points)
    kd_range = -np.logspace(np.log10(abs(initial_kd) / range), np.log10(abs(initial_kd) * range), num_points+2)

    param_combinations = list(itertools.product(kp_range, ki_range, kd_range))

    results = []

    for kp, ki, kd in tqdm(param_combinations, desc="Grid Search Progress"):
        # Set the PID parameters
        controller_params = (kp, ki, kd)
        controller_type = 'pid_tuned'

        # Run the rollout with the current PID parameters
        run_rollout_partial = partial(run_rollout, controller_type=controller_type, model_path=model_path, debug=debug, controller_params=controller_params)
        files = sorted(data_path.iterdir())[:num_segs]
        rollout_results = process_map(run_rollout_partial, files, max_workers=16, chunksize=10)

        # Aggregate costs
        costs = [result[0] for result in rollout_results]
        costs_df = pd.DataFrame(costs)

        # Calculate average costs
        avg_lataccel_cost = np.mean(costs_df['lataccel_cost'])
        avg_jerk_cost = np.mean(costs_df['jerk_cost'])
        avg_total_cost = np.mean(costs_df['total_cost'])
        ...
        results.append({
            ...
            'ki': ki,
            'kd': kd,
            'avg_lataccel_cost': avg_lataccel_cost,
            'avg_jerk_cost': avg_jerk_cost,
...
if __name__ == "__main__":
    ...
    results = grid_search_pid(Path(args.model_path), data_path, args.num_segs, args.debug)

    # Convert results to DataFrame for easier analysis
    results_df = pd.DataFrame(results)

    # Save results to CSV
    results_df.to_csv('pid_grid_search_results.csv', index=False)
